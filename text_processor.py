...
def init_preprocess_text(text):
    ...
    # deleting all tabs, spaces, periods, commas, parentheses and new lines
...
programming_languages = ["python", "java", "cplusplus", "csharp", "javascript", "php",
...
def get_relevancy_of_words(sample):
    ...
    # Replace special characters in the input text
    sample = sample.replace("c#", "csharp").replace("c++", "cplusplus")
...
# function to make storage of bigrams in each row element of dataframe simpler
def cleanup_bigrams_col(big_lst):
    # converting to a list within list structure from tuples within list
    nested_lists = [[t[0], t[1]] for t in big_lst]

    # converting to strings within list structure form list within list
    nested_strings = [' '.join(l) for l in nested_lists]

    return(nested_strings)

def load_and_clean_model(query):
    query = query.lower().strip().replace(" ", "_") + '.csv'
    
    test = pd.read_csv(query).dropna()
    ...
    test['bigrams'] = test['bigrams'].apply(cleanup_bigrams_col)
    
    # grabbing current working directory and making path to folder
    cwd = os.getcwd()
    data_folder = os.path.join(cwd, "data")

    # Create a path to the CSV file within the "data" folder
    query = 'clean_'+ query
    csv_path = os.path.join(data_folder, query)

    # saving cleaned dataset to data folder with prefix "clean_"
